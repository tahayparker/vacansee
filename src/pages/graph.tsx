...
const daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']; // Your array starts Monday=0
const timeIntervals = [
    "08:30",
    "09:00",
    "09:30",
...
export default function GraphPage() {
  ...
  const [selectedDayIndex, setSelectedDayIndex] = useState<number>(() => getAdjustedDayIndex());
  ...
  const tableContainerVariants = {
      hidden: { opacity: 0 },
      visible: { opacity: 1, transition: { delay: 0.1, duration: 0.3, ease: "easeOut" } }, // Faster transition
  ...
  return (
    ...
    >
       ...
       >
            <h1 className="text-3xl md:text-4xl font-bold text-center md:text-left text-white flex-shrink-0"> Room Availability Graph </h1>
             {/* Day Selector Dropdown */}
             <div className="flex items-center justify-center md:justify-end gap-2 flex-grow">
                <label htmlFor="day-select" className="text-sm font-medium text-gray-300 hidden sm:block">Select Day:</label>
                 <Select value={selectedDayIndex.toString()} onValueChange={(value) => setSelectedDayIndex(parseInt(value, 10))} >
                    <SelectTrigger id="day-select" className="w-full sm:w-[180px] bg-black/20 border-white/20 text-white focus:ring-purple-500 focus:border-purple-500">
                        <SelectValue placeholder="Select a day" />
                    </SelectTrigger>
                    <SelectContent className="bg-black/80 backdrop-blur-md border-white/20 text-white">
                        {daysOfWeek.map((day, index) => ( <SelectItem key={index} value={index.toString()} className="focus:bg-purple-600/30 focus:text-white" > {day} </SelectItem> ))}
                    </SelectContent>
                 </Select>
            </div>
       </motion.div>
        ...
        <AnimatePresence mode="wait">
            {isLoading ? (
                 <motion.div key="loader-graph" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-grow items-center justify-center pt-10"> <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-400"></div> </motion.div>
            ) : error ? (
                 <motion.div key="error-graph" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full max-w-7xl mx-auto px-4 py-10 text-center pt-10">
                ...
                <motion.div
                    ...
                    <div className="w-full overflow-auto flex-grow min-h-0 hide-scrollbar border-l border-t border-b border-white/15 rounded-lg shadow-lg bg-black/20 backdrop-blur-sm">
                        <table className="border-separate border-spacing-0 w-full min-w-[1400px]">
                            <thead className="sticky top-0 z-30">
                                <tr>
                                    <th className="sticky left-0 top-0 bg-black text-white z-40 px-3 py-3 border-r border-b border-white/15 text-right text-sm font-semibold whitespace-nowrap"> Room </th>
                                    {timeIntervals.map((time, index) => (
                                        <th key={time} className={`sticky top-0 bg-black text-white z-30 px-3 py-3 border-b border-white/15 text-center text-xs md:text-sm font-medium whitespace-nowrap ${ index === timeIntervals.length - 1 ? '' : 'border-r border-white/15' }`} style={{ minWidth: '65px' }} > {time} </th>
                                    ))}
                                </tr>
                            </thead>
                            ...
                            <tbody className="relative z-0">
                                <AnimatePresence initial={false}>
                                    ...
                                    .sort((a, b) => { const roomA = getRoomName(a?.room) ?? ''; const roomB = getRoomName(b?.room) ?? ''; return roomA.localeCompare(roomB); })
                                    .map((roomData, roomIndex) => {
                                        if (!roomData || typeof roomData.room !== 'string') return null;
                                        return (
                                            ...
                                            >
                                                ...
                                                {/* Data Cells */}
                                                {Array.isArray(roomData.availability) && roomData.availability.map((avail, idx) => (
                                                    <td key={idx} className={`relative z-0 border-b border-black/50 ${getCellColor(avail)} transition-colors duration-150 group-hover:brightness-110 ${ idx === roomData.availability.length - 1 ? '' : 'border-r border-black/50' }`} title={`${getRoomName(roomData.room)} - ${timeIntervals[idx]}`} style={{ minWidth: '65px' }} >
                 ...
                 <motion.p key="empty-graph" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center text-gray-400 py-10 px-4">
