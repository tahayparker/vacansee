...
// --- SVG Icons --- (Keep as is)
export function Google(props: JSX.IntrinsicAttributes & SVGProps<SVGSVGElement>) {
  return (
...
export default function LoginPage() {
  ...
  const getRedirectPathFromQuery = (): string => { /* ... */ const nextPath = router.query.next; if (typeof nextPath === 'string' && nextPath.startsWith('/')) { return nextPath; } return '/';};
  ...
  const handleOAuthLogin = async (provider: Provider) => {
    ...
    if (redirectPath && redirectPath !== '/') {
        Cookies.set('supabase-redirect-path', redirectPath, { path: '/', expires: 1/288 });
        console.log('Stored redirect path in cookie:', redirectPath);
    } else { Cookies.remove('supabase-redirect-path', { path: '/' }); }
    ...
    if (error) {
      ...
      setErrorMessage(`Failed to start login with ${provider}: ${error.message}. Please try again.`);
  ...
  useEffect(() => { /* ... existing useEffect for session check ... */
        const checkSession = async () => { const { data: { session } } = await supabase.auth.getSession(); if (session) { const cookieRedirectPath = Cookies.get('supabase-redirect-path'); const finalRedirectUrl = cookieRedirectPath || '/'; Cookies.remove('supabase-redirect-path', { path: '/' }); console.log('User already logged in (useEffect check), redirecting to:', finalRedirectUrl); router.replace(finalRedirectUrl); } };
        checkSession();
        const { data: authListener } = supabase.auth.onAuthStateChange( (event: string, session: Session | null) => { if (event === 'SIGNED_IN' && session) { const cookieRedirectPath = Cookies.get('supabase-redirect-path'); const finalRedirectUrl = cookieRedirectPath || '/'; Cookies.remove('supabase-redirect-path', { path: '/' }); console.log('Auth state changed to SIGNED_IN (listener), redirecting to:', finalRedirectUrl); router.replace(finalRedirectUrl); } else if (event === 'SIGNED_OUT') { console.log('Auth state changed to SIGNED_OUT (listener)'); } } );
