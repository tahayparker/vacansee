...
type ResponseData = {
    available: boolean;
    checked: { // Include checked parameters for clarity
        roomName: string;
        day: string;
        startTime: string;
        endTime: string;
...
export default async function handler(
  ...
  console.log('[API Check Availability] Received:', { roomName, day, startTime, endTime });

  if (!roomName || !day || !startTime || !endTime) {
    return res.status(400).json({ error: 'Missing required fields: roomName, day, startTime, endTime' });
  }
  ...
  try {
    ...
    if (isAvailable) {
      console.log(`[API Check Availability] Room "${roomName}" is AVAILABLE on ${day} from ${startTime} to ${endTime}`);
      return res.status(200).json({ available: true, checked: checkedParams });
    } else {
      console.log(`[API Check Availability] Room "${roomName}" is NOT AVAILABLE on ${day} from ${startTime} to ${endTime}. Conflicts:`, conflicts.length);
