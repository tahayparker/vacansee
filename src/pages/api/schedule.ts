...
export default async function handler(
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
        return res.status(500).json({ error: 'Failed to parse schedule data: Invalid JSON format.' });
    }
    return res.status(500).json({ error: 'Internal Server Error reading schedule data' });
  }
}