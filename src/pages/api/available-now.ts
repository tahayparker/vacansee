...
export default async function handler(
    ...
    try {
        ...
        console.log(`[API Available Now] Using Luxon. Checking for Day: ${currentDayNameDubai}, Time: ${currentTimeStringDubai} in ${DUBAI_TIMEZONE}`);
        ...
        const bookedRoomNames = bookedTimings.map((timing: { Room: string }) => timing.Room);
        ...
        const initialRooms: AvailableRoomInfo[] = availableRoomsData.map(room => ({
            name: room.Name, shortCode: room.ShortCode, capacity: room.Capacity
        }));
        ...
        console.log('[API Available Now] Filtered rooms count:', filteredRooms.length);
        ...
        if (error.code) { console.error(`[API Available Now] Prisma Error Code: ${error.code}`); }
        return res.status(500).json({ error: error.message || "Internal Server Error" });
    } finally {
        await prisma.$disconnect().catch(e => console.error("[API Available Now] Error disconnecting Prisma:", e));
    }
}